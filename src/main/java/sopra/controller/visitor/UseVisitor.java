...
public class UseVisitor extends DefaultEntityVisitor {
  ...
  protected void handle(final Weapon weapon) {
    switch (this.player.getState()) {
      case DEFAULT, ATTACKED -> {
        ...
        this.player.addItem(this.index, current); // null pointer
        if (current.getEffect() != null) {
          player.addToSkill(CharacterSkill.valueOf(current.getEffect().toUpperCase(Locale.ENGLISH)),
              -1);
        }
        this.player.setWeapon(weapon);
        if (weapon.getEffect() != null) {
          player.addToSkill(CharacterSkill.valueOf(weapon.getEffect().toUpperCase(Locale.ENGLISH)),
              1);
        }
        this.observers.forEach(observer -> observer.notifyUpdatePlayer(this.player));
      }
  ...
  protected void handle(final Armor armor) {
    switch (this.player.getState()) {
      case DEFAULT, ATTACKED -> {
        this.player.removeItem(this.index);
        if (this.player.getArmor().isPresent()) {
          final Armor current = this.player.getArmor().get();
          if (current.getEffect() != null) {
            player
                .addToSkill(CharacterSkill.valueOf(current.getEffect().toUpperCase(Locale.ENGLISH)),
                    -1);
          }
        ...
        if (armor.getEffect() != null) {
          player
              .addToSkill(CharacterSkill.valueOf(armor.getEffect().toUpperCase(Locale.ENGLISH)), 1);
        }
        this.observers.forEach(observer -> observer.notifyUpdatePlayer(this.player));
      }
      case COMBAT -> this.observers
  ...
  protected void handle(final Recipe recipe) {
    this.observers
        .forEach(observer -> observer.notifyCommandFailed("Use not possible for this Item."));
  }
