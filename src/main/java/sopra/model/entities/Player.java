...
public class Player extends Character implements JSONSerializable<JSONObject> {
  ...
  private Player(final String name, final int level, final int experience, final int skillPoints,
      final int maxHealth, final int currentHealth, final EnumMap<CharacterSkill, Integer> skills,
      final Weapon weapon, final Optional<Armor> armor) {
    super(EntityType.PLAYER, name, level, currentHealth, maxHealth, weapon, armor, skills,
  ...
  public void removeAll(final int index) {
    this.inventory.removeStack(index);
  }
  ...
  private static class Builder {
    ...
    Player build() {
      ...
      return new Player(this.name, this.level, this.experience, this.skillPoints, maxHealth,
    ...
    public void setArmor(final Armor armor) {
      ...
      if (armor != null && armor.getEffect() != null) {
        final CharacterSkill tmp = CharacterSkill
            .valueOf(armor.getEffect().toUpperCase(Locale.ENGLISH));
        this.skills
            .put(tmp, skills.get(tmp) + 1);
      }
    }
    ...
    public void setWeapon(final Weapon weapon) {
      this.weapon = weapon;
      if (weapon != null && weapon.getEffect() != null) {
        final CharacterSkill tmp = CharacterSkill
            .valueOf(weapon.getEffect().toUpperCase(Locale.ENGLISH));
        this.skills
            .put(tmp, skills.get(tmp) + 1);
      }
    }
